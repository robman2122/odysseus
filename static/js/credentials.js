...
export const credentials = {
    ...
    init() {
        ...
        const railBtn = document.getElementById('rail-secrets'); // Placeholder for future rail icon
        ...
        this.list.innerHTML = creds.map(c => `
            <div class="secret-item" style="display:flex; align-items:center; gap:12px; padding:10px; background:color-mix(in srgb, var(--fg) 3%, transparent); border:1px solid var(--border); border-radius:4px;">
                <div style="flex-grow:1; min-width:0;">
                    <div style="font-weight:600; font-size:13px; font-family:monospace; overflow:hidden; text-overflow:ellipsis;">${c.name}</div>
                    <div style="font-size:11px; opacity:0.5; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${c.description || 'No description'}</div>
                </div>
                <div style="display:flex; gap:4px;">
                    <button class="btn-icon" onclick="window._odysseusCredentials.edit('${c.id}', '${c.name.replace(/'/g, "\\'")}', '${(c.description || '').replace(/'/g, "\\'")}')" title="Edit">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    <button class="btn-icon danger" onclick="window._odysseusCredentials.delete('${c.id}')" title="Delete">
