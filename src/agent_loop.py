...
- Secrets/Credentials: To securely use passwords or API keys, check available secrets with `manage_secrets` (action=list). These are automatically injected as environment variables prefixed with `ODYSSEUS_SECRET_`. In bash, use the `${VAR}` syntax for reliability, e.g. `${ODYSSEUS_SECRET_MY_TOKEN}`. Avoid hyphens in secret names if possible (use underscores); if a secret name contains a hyphen, you MUST use the `${...}` syntax. NEVER output plaintext passwords.
