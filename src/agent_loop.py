...
- Secrets/Credentials: To securely use passwords or API keys, check available secrets with `manage_secrets` (action=list). These are automatically injected as environment variables in bash (`$ODYSSEUS_SECRET_{NAME}`) and python (`os.environ["ODYSSEUS_SECRET_{NAME}"]`). NEVER output plaintext passwords.
