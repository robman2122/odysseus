...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_secrets": "Secret Vault management: list available user secrets (passwords, API keys). Use to check what secrets are available as ODYSSEUS_SECRET_{NAME} environment variables before executing bash/python scripts that require credentials. Do NOT output plaintext secrets.",
